...
def external_data_reco_dm(data_path, external_data) -> pd.DataFrame:
    """
    ...
    dm_external_data = external_data[["SUBJID", "LBREFID", "YOB", "SEX","VISIT"]].drop_duplicates(keep='first')
    # 外部数据中的YOB原本是Float类型
    ...
    data["YOB_trans"] = data.apply(
        lambda x: str(int(x["ICDT_Y"]) - x["YOB"]) if pd.notnull(x["ICDT_Y"]) and pd.notnull(x["YOB"]) else None,
        axis=1)
